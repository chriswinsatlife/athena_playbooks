...
function parseQueryParams() {
  ...
  // Handle data file parameter
  if (params.has('data')) {
    const dataParam = params.get('data');
    if (dataParam === 'large') {
      config.dataFile = 'data_large.json';
    } else if (dataParam === 'small') {
      config.dataFile = 'data_small.json';
    } else if (dataParam.endsWith('.json')) {
      config.dataFile = dataParam;
    }
  }
...
if (config.debug) {
  ...
  console.log('  ?data=small    - Load data_small.json');
  console.log('  ?data=large    - Load data_large.json'); 
  console.log('  ?data=file.json - Load custom file');
  console.log('  ?debug=true    - Enable debug logging');
  console.log('  ?theme=name    - Set theme (future use)');
}
...
d3.json(config.dataFile).then(data => {
  ...
  console.error('Error loading data file:', config.dataFile, error);
  ...
  svg.append("text")
    ...
    .attr("fill", "red")
    .attr("font-size", "16px")
    .text(`Error loading ${config.dataFile}. Available files: data.json, data_small.json, data_large.json`);
  ...
  svg.append("text")
    ...
    .attr("fill", "gray")
    .attr("font-size", "14px")
    .text("Usage: ?data=small or ?data=large or ?data=myfile.json");
}); 